...
def hexify(payload):
    ...
    payload_hex_padded = [
        val if len(val) > 1 else '0' + val for val in payload_hex
    ]
    payload_hex_padded = ''.join(payload_hex_padded)
    len_delimited_payload = "0x" + \
        encode(payload_length, return_type="hex")[0] + payload_hex_padded
    return len_delimited_payload


def dehexify(payload):
    """
        >> dehexify(Ox68656c6c6f2c20776f726c64a)
        >> 'hello, world'
    """
    pass
